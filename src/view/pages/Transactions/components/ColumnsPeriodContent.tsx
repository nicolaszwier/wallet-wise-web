...
export function ColumnsPeriodContent({ dateRange, isLoading, period, onSelectItem, onEditItem, onPayItem, onDeleteItem }: ComponentProps) {
      ...
      {isLoading ? 
        ...
        <>
          <div className="flex-grow p-1">
            ...
            {period?.transactions.length === 0 || !period && !isLoading && (
          ...
          <div className="flex justify-between p-1 pr-2 align-middle items-center">
            ...
            <div className="text-sm font-semibold pl-2 text-red">{formatCurrency(sumPendingTransactionsByType(period?.transactions ?? [], TransactionType.EXPENSE), selectedPlanning?.currency ?? 'BRL', i18n.language)}</div>
          </div>
          <div className="flex justify-between p-1 pr-2 align-middle items-center">
            <div className="text-xs text-muted-foreground pl-2">
              {t('transactions.periodFooterIncomePending')}
            </div>
            <div className="text-sm font-semibold pl-2 text-green">{formatCurrency(sumPendingTransactionsByType(period?.transactions ?? [], TransactionType.INCOME), selectedPlanning?.currency ?? 'BRL', i18n.language)}</div>
          </div>
          <div className="flex justify-between p-1 pr-2 align-middle items-center">
            <div className="text-xs text-muted-foreground pl-2">
